...
sed -i 's/^MODULES=(/MODULES=(virtio virtio_ring virtio_mmio virtio_blk virtio_net virtio_console squashfs ext4 /' /etc/mkinitcpio.conf
sed -i 's/^HOOKS=.*/HOOKS=(base udev modconf block bindboot filesystems)/' /etc/mkinitcpio.conf
mkinitcpio -P
